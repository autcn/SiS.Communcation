...
namespace SiS.Communication
{
    ...
    public class SimpleRingQueue
    {
        ...
        public SimpleRingQueue(int capacity, int maxCount)
        {
        ...
        /// </summary>
        public SimpleRingQueue() : this(8 * 1024, int.MaxValue)
        {
        ...
        private bool AllocBuffer(int requiredTotalLen)
        {
            ...
            for (int i = 0; ; i++)
            {
                ...
                if (newLength >= requiredTotalLen)
                {
                    ...
                    if (_dataCount > 0)
                    {
                        Buffer.BlockCopy(_buffer, _startIndex, newBuffer, 0, FirstDataCount);
                        if (SecondDataCount > 0)
                        {
                            Buffer.BlockCopy(_buffer, 0, newBuffer, FirstDataCount, SecondDataCount);
                        }
                    }
                    _startIndex = 0;
                    _buffer = newBuffer;
        ...
        private int Read(byte[] outBuffer, int offset, int count, bool isPeek)
        {
            ...
            if (readFirstLen > 0)
            {
                Buffer.BlockCopy(_buffer, _startIndex, outBuffer, offset, readFirstLen);
            }

            if (readSecondLen > 0)
            {
                Buffer.BlockCopy(_buffer, 0, outBuffer, offset + readFirstLen, readSecondLen);
            }
            if (!isPeek)
            {
                _dataCount -= count;
        ...
        /// <returns>True if the data lenght less than limit size; otherwise false.</returns>
        public virtual bool Write(byte[] data, int offset, int count)
        {
            ...
            if (firstSpace == 0)
            {
                Buffer.BlockCopy(data, offset, _buffer, SecondDataCount, count);
            }
            else
            {
                int firstWriteLen = 0;
                ...
                if (firstWriteLen > 0)
                {
                    Buffer.BlockCopy(data, offset, _buffer, _startIndex + _dataCount, firstWriteLen);
                }
                if (secondWriteLen > 0)
                {
                    Buffer.BlockCopy(data, offset + firstWriteLen, _buffer, SecondDataCount, secondWriteLen);
                }
            }
